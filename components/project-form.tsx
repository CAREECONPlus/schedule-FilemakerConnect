...
import { Upload, X, Search, Database, CalendarIcon, ChevronLeft, ChevronRight, Clock } from "lucide-react"
...
export function ProjectForm({ onSubmit }: ProjectFormProps) {
  ...
  const [showInGlobalCalendar, setShowInGlobalCalendar] = useState(true)
  ...
  useEffect(() => {
    ...
    if (savedData) {
      try {
        ...
        setWorkStartTime(parsed.workStartTime || "")
        setWorkEndTime(parsed.workEndTime || "")
        setShowInGlobalCalendar(parsed.showInGlobalCalendar !== undefined ? parsed.showInGlobalCalendar : true)
      } catch (error) {
                ...
                <div className="space-y-3 p-4 bg-secondary/20 rounded-lg border-2 border-dashed border-secondary">
                  ...
                  {workStartTime && workEndTime && (
                    <div className="text-xs text-primary font-medium">
                      作業時間: {workStartTime} 〜 {workEndTime}
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-6 pt-6 border-t-2 border-dashed border-muted">
              ...
              <div className="space-y-3 p-4 bg-accent/10 rounded-lg border-2 border-accent/30">
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="globalCalendar"
                    checked={showInGlobalCalendar}
                    onCheckedChange={(checked) => setShowInGlobalCalendar(checked as boolean)}
                  />
                  <Label htmlFor="globalCalendar" className="text-sm cursor-pointer">
                    全体カレンダーに表示する
                  </Label>
                </div>
